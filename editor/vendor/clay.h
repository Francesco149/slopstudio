...
// [slopstudio local patch] Relaxed the C++ floor from C++20 (202002L) to C++17 (201703L).
// Clay wants C++20 only for its CLAY()/CLAY_TEXT() convenience macros (compound-literal
// designated inits). slopstudio's editor (C++17) does NOT use those macros — it drives Clay
// via the imperative Clay__OpenElement/ConfigureOpenElement/CloseElement API and builds the
// config structs field-by-field, and the implementation TU (clay_impl.c) compiles as C99.
// So C++17 is sufficient for our usage. Re-apply this hunk when re-vendoring clay.h.
#if !( \
    (defined(__cplusplus) && __cplusplus >= 201703L) || \
    (defined(__STDC__) && __STDC__ == 1 && defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) || \
    defined(_MSC_VER) \
)
#error "Clay requires C99, C++17 (slopstudio-relaxed), or MSVC"
#endif
