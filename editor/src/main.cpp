...
static void scan_library() {
    ...
    for (LibType t : {LIB_IMAGE, LIB_AUDIO, LIB_VIDEO}) {
        std::string dir = g_libraryDir + "/" + lib_subdir(t);
        ensure_dir(dir);
        WIN32_FIND_DATAW fd;
        HANDLE h = FindFirstFileW(to_w(dir + "/*").c_str(), &fd);
...
static AvatarFace avatar_face(const std::string& uri) {
    static std::map<std::string, AvatarFace> cache;
    auto it = cache.find(uri);
    if (it != cache.end()) return it->second;
    const SrcPix* sp = uri.empty() ? nullptr : get_src_pixels(uri);
    AvatarFace fa = (sp && sp->w > 0) ? face_from_pixels(sp->px.data(), sp->w, sp->h) : AvatarFace{};
    cache[uri] = fa;
    return fa;
}
...
static Wave* get_wave(const std::string& uri, int buckets = 1024) {
    ...
    if (read_wav16(resolve_asset(uri), mono, srate) && srate > 0 && !mono.empty()) {
        size_t nframes = mono.size();
        w.dur = (double)nframes / (double)srate;
...
// the downsampled waveform envelope. WAV only (the editor has no mp3 decoder; export hands
// non-WAV audio to ffmpeg).
...
static Pcm* get_pcm(const std::string& uri) {
    ...
    Pcm pc;
    if (read_wav16(resolve_asset(uri), pc.mono, pc.srate) && pc.srate > 0 && !pc.mono.empty()) {
...
static Pcm* get_video_pcm(const std::string& srcPath) {
    ...
    Pcm pc;
    g_videoPcmCache[srcPath] = pc;                       // negative-cache up front
    AVFormatContext* fmt = nullptr;
    if (avformat_open_input(&fmt, srcPath.c_str(), nullptr, nullptr) < 0) return nullptr;
...
// Audio clips = clips on rows under a track of kind "audio" that have a ready WAV asset.
static std::vector<MixSrc> collect_audio(const Project& p) {
    std::vector<MixSrc> v;
    ...
    for (auto& kv : p.clips) {
        ...
        std::string sfx = uri.substr(uri.size() - 4);
        for (auto& ch : sfx) ch = (char)((ch >= 'A' && ch <= 'Z') ? ch + 32 : ch);
        if (sfx != ".wav") continue;                          // editor decodes WAV only
        Pcm* pc = get_pcm(uri);
        if (!pc) continue;
        ...
        double rate = c.params.value("rate", 1.0);    // playback speed (pitch-preserved time-stretch)
        ...
        v.push_back({pc, c.start, c.dur, gain, in_off});
    }
...
// (linear-resampled, gained), clip, write interleaved 16-bit.
static void audio_fill(const std::vector<MixSrc>& srcs, int16_t* out, int frames, double t0) {
    std::vector<float> mix(frames, 0.f);
    for (auto& s : srcs) {
        const std::vector<float>& m = s.pcm->mono;
        for (int j = 0; j < frames; ++j) {
            double tau = t0 + (double)j / AUD_RATE - s.start;     // time since the clip's start
            ...
            mix[j] += (a + (b - a) * fr) * s.gain;
        }
    }
    for (int j = 0; j < frames; ++j) {
        float v = mix[j]; v = v > 1.f ? 1.f : (v < -1.f ? -1.f : v);
...
static void audio_pump(const Project& p) {
    ...
    for (int i = 0; i < AUD_NBUF; ++i) {
        ...
        audio_fill(srcs, g_woBuf[i].data(), AUD_FRAMES, g_audioFillT);
...
static bool clip_native_size(Project& p, Clip& c, float& nw, float& nh) {
    if (c.type == "image") {
        auto au = p.asset_uri.find(c.asset);
        if (au != p.asset_uri.end()) { Tex* t = get_texture(resolve_asset(au->second)); if (t && t->w > 0) { nw = (float)t->w; nh = (float)t->h; return true; } }
        ...
        if (vm != p.asset_video.end()) {
            ...
            if (m.w > 0) { nw = (float)m.w; nh = (float)m.h; return true; }
        }
...
static float layout_scale_mul(Project& p, Clip& c) {
    ...
    if (lay.rfind("inset", 0) == 0) {
        float L = std::min(0.55f * FW / nw, 0.72f * FH / nh);
...
static void draw_caption_clip(ImDrawList* dl, float cx, float cy, float s, Clip& c, int alpha, double t,
    ...
    if (autoCorner >= 0 && fw > 1) {
        ...
        x0 = ((autoCorner & 1) == 0) ? f0.x + mx : f0.x + fw - W - mx;
        y0 = (autoCorner >= 2) ? f0.y + fh - H - fh * 0.045f : f0.y + my;
    }
...
static TransFx clip_transition(Project& p, Clip& c, double T) {
    ...
    if (tr.is_boolean() && !tr.get<bool>()) return fx;                 // transition:false
    ...
    std::string inType, outType; double inDur, outDur;
    spec("in", inType, inDur); spec("out", outType, outDur);
    ...
    if (inType == "fade" && !(tr.is_object() && tr.contains("in"))) {
        if (c.type == "image" || c.type == "video") {
            std::string lay = jstr(c.params, "layout");
            if (lay == "fit" || lay.rfind("inset", 0) == 0) inType = "pop";
        } else if (c.type == "diagram") inType = "pop";   // diagram cards enter like image showcases
    }
    double half = c.dur * 0.5;
    inDur = std::min(inDur, half); outDur = std::min(outDur, half);   // short clip → proportional
    ...
    bool skipIn = false, skipOut = false;
    ...
    double ctol = (rit != p.rows.end() && rit->second.type == "avatar") ? 0.35 : CONTIG;
    if (rit != p.rows.end())
        for (auto& oid : rit->second.clips) {
            ...
            if (std::fabs(oe - c.start) < ctol) { skipIn = true; nbIn = &oit->second; }            // neighbour ends ~at our start
            if (std::fabs(os - (c.start + c.dur)) < ctol) { skipOut = true; nbOut = &oit->second; } // neighbour starts ~at our end
        }
    ...
    if (rit != p.rows.end() && rit->second.type == "avatar") {
        ...
        const double SW = 0.20, DIST = 640.0;              // per-side duration (s); slide depth (project px)
        std::string mySig = sig(c);
        if (nbOut && sig(*nbOut) != mySig && tout < SW) {  // slide OUT — accelerates toward the edge
        ...
        if (nbIn && sig(*nbIn) != mySig && tin < SW) {     // slide IN — decelerates into place
            double u = 1.0 - tin / SW;
            fx.dy += (float)(DIST * u * u);
            fx.vy -= (float)(DIST * 2.0 * u / SW);
        ...
        const double GLIDE = 0.35;
        if (nbIn && sig(*nbIn) == mySig && tin < GLIDE) {
            double gdx = (double)nbIn->tx_pos[0] - c.tx_pos[0];
            double gdy = (double)nbIn->tx_pos[1] - c.tx_pos[1];
            if (std::fabs(gdx) + std::fabs(gdy) > 2.0) {
                double u = 1.0 - tin / GLIDE;
                fx.dx += (float)(gdx * u * u);           fx.dy += (float)(gdy * u * u);
                fx.vx -= (float)(gdx * 2.0 * u / GLIDE); fx.vy -= (float)(gdy * 2.0 * u / GLIDE);
            }
        }
    }
    ...
    bool popThrough = skipIn && inType == "pop" && (c.type == "image" || c.type == "video");
    if ((!skipIn || popThrough) && inType != "none") {
        if (!skipIn && inDur > 1e-3 && tin < inDur) {
            double e = ease(tin / inDur);
            if (!hasOpKf) fx.alphaMul *= (float)e;                              // fade in
            if (inType != "fade" && inType != "pop") slide(inType, 1.0f - (float)e, fx, false);
        }
        if (inType == "pop")                                                   // SPRING pop-in (own timescale)
            fx.sclMul *= (float)(POP_MIN + (1.0 - POP_MIN) * spring_response(tin, POP_K, POP_C));
    }
    if (!skipOut && outType != "none" && outDur > 1e-3 && tout < outDur) {
        double e = ease(tout / outDur);
        if (!hasOpKf) fx.alphaMul *= (float)e;                                 // fade out
        if (outType == "pop")       fx.sclMul *= POP_MIN + (1.0f - POP_MIN) * (float)e;  // clean shrink-out
        else if (outType != "fade") slide(outType, 1.0f - (float)e, fx, true);
    }
    return fx;
}
        ...
        for (auto& rid : tk->rows) {
            ...
            for (auto& cid : rit->second.clips) {
                ...
                if (c.type == "caption" || c.type == "text") {
                    ...
                    // lower_third with a default transform = the bottom STRAP, pinned to the bottom
                    // corner AWAY from the host/content (a centered strap sits right on a centered
                    // host's face — the user's "raki masu plate over her face"). Explicit pos/place wins.
                    if (place.empty() && defaultPos && capStyle == "lower_third") place = "strap";
                ...
                if (c.type == "avatar") {
                    ...
                    // PRESENTER layout: with content on screen and the clip's horizontal pos at the default,
                    // auto-place the host in the OPPOSITE corner (sits aside, facing the thing). pos overrides.
                    float autoOffX = (side != 0 && c.tx_pos[0] == 0.0) ? -side * fw * 0.24f : 0.0f;
                    ...
                    if (autoOffX == 0.0f && side == 0 && c.tx_pos[0] == 0.0)
                        for (auto& kv : p.clips) {
                            const Clip& oc = kv.second;
                    ...
                    float talk = !haveVO ? 0.0f
                               : voWave ? (float)audio_talk_at(voWave, st.playhead - visOrigin, atk, dec)
                                        : (float)viseme_talk_at(vtrack, st.playhead - visOrigin, atk, dec);
                    ...
                    if (sp) {
                        ...
                        // Contact shadow at her FEET — only for NON-floating sprites shown full-body
                        // (a floating pose hovers; bust/closeup have feet off-frame). Positioned at the
                        // alpha-bbox bottom (her actual feet), not the sprite-quad bottom. `shadow` adjusts.
                        if (shadow > 0.01 && !spriteFloating && !faceShot) {
                            ImVec4 bb;
                            if (sprite_bbox(spPath, bb) && sp->w > 0 && sp->h > 0) {
                                float fX0 = std::min(a.x, b.x), fW = fabsf(b.x - a.x), fH = b.y - a.y;
                                float feetY  = a.y + (bb.w + 1) / (float)sp->h * fH;             // bbox bottom → screen
                                float footCx = fX0 + ((bb.x + bb.z) * 0.5f) / (float)sp->w * fW; // bbox center-x → screen
                                float sw2 = (bb.z - bb.x + 1) / (float)sp->w * fW * 0.78f, sh = sw2 * 0.16f;
                                ImVec2 sa(footCx - sw2 * 0.5f, feetY - sh * 0.5f), sb(footCx + sw2 * 0.5f, feetY + sh * 0.5f);
                                int salpha = (int)((shadow > 1 ? 1 : shadow) * 0.5 * alpha);
                ...
                if (c.type == "video") {
                    ...
                    if (ft) {
                        float w = ft->w * std::fabs(eSclX) * s, h = ft->h * std::fabs(eSclY) * s;   // |scale| + UV flip = mirror in place
                        ImVec2 a(cx - w * eAncX, cy - h * eAncY), b(a.x + w, a.y + h);
                        ImVec2 uv0(eSclX < 0 ? 1.f : 0.f, eSclY < 0 ? 1.f : 0.f), uv1(eSclX < 0 ? 0.f : 1.f, eSclY < 0 ? 0.f : 1.f);
                        if (c.params.is_object() && c.params.contains("glow"))   // optional outer glow (common look prop)
                            draw_clip_glow(dl, a, b, c.params["glow"], s, alpha / 255.0f);
                ...
                if (tex) {
                    ...
                    float w = tex->w * std::fabs(eSclX) * s;
                    float h = tex->h * std::fabs(eSclY) * s;
...
static std::string add_audio_clip_at(Project& p, const std::string& row, double t, const std::string& path) {
    ...
    if (p.doc.contains("assets") && p.doc["assets"].is_object())
        p.doc["assets"][akey] = json{{"provider", "external"}, {"type", rtype == "tts" ? "speech" : "music"},
                                     {"status", "ready"}, {"uri", path}, {"meta", json::object()}};
...
static void DrawTimeline(Project& p, UIState& st, const std::map<std::string, GenLite>& gen) {
    ...
    for (auto& tk : p.tracks) {
        ...
        for (auto& rid : tk.rows) {
            ...
            if (firstRow) {   // ▲▼ reorder the whole track (higher = drawn on top)
                ImGui::PushID(("trk" + tk.id).c_str());
                ImGui::SetCursorScreenPos(ImVec2(trackX - 34, ly + 4));
    ...
    for (auto& kv : p.clips) {
        ...
        if (ra != rowAudio.end() && ra->second && !c.asset.empty()) {
            ...
            if (wv && !wv->env.empty()) {
                ...
                // split clip shows its slice of the asset (rect-clipped to the clip body).
                double inOff = c.params.value("in", 0.0);
                dl->PushClipRect(a, b, true);
                for (float xx = a.x; xx < b.x; xx += 1.0f) {
                    float at = (float)inOff + (xx - a.x) / pps;   // asset time (s) at this pixel
...
static bool spawn_wsl_bash(const std::string& inner) {
    ...
    std::string cmd = "wsl.exe --cd \"" + std::string(cwd) + "\" -e bash -lc \"" + esc + "\"";
...
static bool spawn_render(const std::string& projPath, int targetMb, int fps, bool p1080, const std::string& out) {
    ...
    std::string inner = "nix develop --command bash tools/export.sh '" + projPath + "'"
...
static std::string add_native_clip_at(Project& p, const std::string& type, double t) {
...
static void DrawUI(Project& p, UIState& st, bool& reload, const std::map<std::string, GenLite>& gen) {
    ...
    if (ImGui::BeginPopupModal("Stock assets missing", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("Fetch now", ImVec2(130, 0))) {
            ...
            snprintf(smsg, sizeof smsg, ok ? "Fetching in a console window — reload the project when it finishes."
                                           : "Could not launch wsl.exe. Run: python tools/fetch-stock-assets.py");
            g_stockChecked = 2;
        }
        ImGui::SameLine();
        if (ImGui::Button("Skip", ImVec2(130, 0))) { g_stockChecked = 2; smsg[0] = 0; ImGui::CloseCurrentPopup(); }
        ImGui::EndPopup();
    }
    ...
    if (ImGui::BeginPopupModal("Render video", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        ImGui::Separator();
        ImGui::Checkbox("Scale to 1080p", &p1080);
        ...
        ImGui::TextDisabled("space-efficient 1-pass x264 ABR (YouTube re-encodes anyway).");
        ...
        if (msg[0]) { ImGui::Separator(); ImGui::TextWrapped("%s", msg); }
        ImGui::Separator();
        if (ImGui::Button("Render", ImVec2(120, 0))) {
            bool ok = spawn_render(p.path, targetMb, fps, p1080, out);
            snprintf(msg, sizeof msg, ok ? "Rendering in a new console window → %s\n(close this dialog; it keeps running)"
                                         : "Could not launch wsl.exe. Run manually:\n"
                                           "nix develop --command bash tools/export.sh %s --target-mb %d --fps %d%s --out %s",
                     out.c_str(), p.path.c_str(), targetMb, fps, p1080 ? " --scale 1080" : "", out.c_str());
        }
        ImGui::SameLine();
        if (ImGui::Button("Close", ImVec2(120, 0))) { msg[0] = 0; ImGui::CloseCurrentPopup(); }
        ImGui::EndPopup();
    }
    ...
    if (ImGui::BeginTabBar("right_panes")) {
        ...
        if (!st.selected.empty() && it != p.clips.end()) {
            ...
            // ── audio: per-clip volume · per-lane (track) volume · loudness normalize · audition ──
            if (c.type == "tts" || c.type == "music") {
                ImGui::SeparatorText("audio");
                float volc = (float)c.params.value("gain_db", 0.0);
                ImGui::SetNextItemWidth(180);
                if (ImGui::SliderFloat("clip volume (dB)", &volc, -24.f, 24.f, "%+.1f")) c.params["gain_db"] = (double)volc;
                auto rit = p.rows.find(c.row);
                if (rit != p.rows.end()) {
                    float volr = (float)rit->second.params.value("gain_db", 0.0);
                    ImGui::SetNextItemWidth(180);
                    if (ImGui::SliderFloat("lane volume (dB)", &volr, -24.f, 24.f, "%+.1f")) {
                ...
                float rate = (float)c.params.value("rate", 1.0);
                ImGui::SetNextItemWidth(180);
                if (ImGui::SliderFloat("speed (rate)", &rate, 0.5f, 2.0f, "%.2fx")) {
                    double oldRate = c.params.value("rate", 1.0);
                ...
                if (au != p.asset_uri.end()) uri = au->second;
                if (!uri.empty()) {
                    std::string apath = resolve_asset(uri);
...
static void write_export_plan(Project& p, const std::string& path, int W, int H, int fps, int frames, double dur) {
    ...
    plan["audio"] = audio;
    json credits = json::array();  // auto-credits from every sourced asset's attribution
    ...
    for (auto& kv : assetsj.items()) {
        ...
        if (att.is_object()) {
            ...
            if (!txt.empty()) credits.push_back(txt);
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (!shotFramePath.empty()) {  // headless: ONE full-res composite frame → PNG (verify/thumbnail)
        int W = (int)proj.width, H = (int)proj.height, rc = 1;
        if (create_export_targets(W, H)) {
            std::vector<unsigned char> buf;
            render_export_frame(proj, initialTime, W, H, buf);
            float sig = 0.f; if (frame_blur_strength(proj, initialTime, sig) > 0.003f) blur_rgba(buf, W, H, sig * (float)W / (float)proj.width);  // whole-frame blur
            if (stbi_write_png(shotFramePath.c_str(), W, H, 4, buf.data(), W * 4)) {
                fprintf(stderr, "shot-frame: %dx%d @ t=%.2fs → %s\n", W, H, initialTime, shotFramePath.c_str());
                rc = 0;
