...
## wslop storage note
`cache/` (gen output — TTS/align/video) + `exports/` (rendered mp4s) live on the **WSL ext4 drive**
(gitignored, regenerable). A past attempt to relocate them to the Windows C: drive (to save WSL vdisk
space + let the Windows editor read cache natively) was **reverted**: the editor bakes its `--cache`
dir into the export PLAN's audio paths, and if that's a native `C:\…` path, **ffmpeg (a WSL process)
can't read it → silent exports**. If you ever relocate again, the plan step MUST use a WSL-readable
cache path so ffmpeg can open the VO wavs; only the editor's own asset reads could take a native path.
...
- **GPU strategy:** the near-term gen stack — **image (Anima/ComfyUI), TTS
