...
"assets": {
  "a_3f9c…": {
    ...
    "meta": {
      ...
      "attribution": null                    // present for sourced music/art
    }
  }
}
```
...
- in-process decode is full-rate from the source (no proxy-fps cap); NVDEC/d3d11va hardware decode can later
  ...
  proxy, its `--fps` caps B-roll smoothness — re-extract higher for the master.) Demo: `examples/video-broll.slop.json`.
...
- **clone vs design:** a preset shipping a golden `ref` clip → CLONE (identical timbre across
  ...
  clip — see `examples/fufu-lab.slop.json` (the hiragana ふふ reads as a laugh; use romaji).
...
- **integrate into a plate — AUTO by default:** an avatar clip samples the bg plate behind it and grades
  ...
  pool at her feet). Project-level **`meta.vignette`** (0..1) darkens frame edges to unify bg + host. The
  "blend the character into the background" combo = a little grade + a contact shadow + a touch of vignette.

**`music`** — `source` (jamendo | ace-step), `mood`, `seed`; emits `attribution` → auto-credits.
