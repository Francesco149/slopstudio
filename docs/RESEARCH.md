...
for posing; **Qwen-Image (Apache-2.0)** for fast clean technical diagrams + legible text.
Engine: **ComfyUI** (HTTP/WS API) — the de-facto extensible, LLM-driveable backend.

> ⚠ **2026-07-03 correction (commercial-safety):** ~~SANA~~ is NOT a safe pick for a
> *monetized, self-hosted* pipeline — the SANA *code* is Apache-2.0 but the **weights
> (`Sana_1600M…`) are NVIDIA "research only"** (NSCL). Swapped to **Qwen-Image (Apache-2.0)**,
> which also renders legible text (useful for thumbnails). Also never wire in the face-ID
> stack (**InstantID / PuLID / IP-Adapter-FaceID + InsightFace `antelopev2`/`buffalo_*` packs**
> — all non-commercial) or the **Impact** font (proprietary). See `../gemma-branding/research/thumbnail-pipeline-2026-07.md` §7.
...
| **Qwen-Image** | ✓/✓/✓ | ★★★ | **Apache-2.0** | clean diagrams + **legible text**; the SANA replacement |
| ~~SANA 0.6B~~ | ✓/✓/✓ | ★★ | code Apache / **weights research-only** | ❌ **excluded** — NVIDIA NSCL weights, not commercial-safe |
