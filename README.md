...
```
  ...
  │  pull-based render graph      │ ◄────── │  (Qwen3-TTS, ComfyUI/Anima,       │
...
./build/slopstudio.exe examples/signature-opener.slop.json    # open a project (Windows / WSLInterop)
```
...
| Host sprite art | cloud GPT image gen → sprite rig (rembg matte + pose tools); local Anima 2B + character LoRA as backup | outputs owned / weights NC · outputs OK |
| Backdrops / anime stills | Anima 2B | weights NC · **outputs commercial-OK** |
| Diagrams / legible text | Qwen-Image | Apache-2.0 |
...
| Avatar | pngtuber sprite state machine (Inochi2D planned, not yet supported) | — |
