...
- **Providers (Python, on lame):** uniform HTTP/WS job protocol
  ...
  - **image** ComfyUI + **Illustrious-XL 2.0** (RAIL-M) + a trained **Gemma-san LoRA**
    for the recurring host; SANA (Apache) for clean diagrams.
  ...
  - AVOID (non-commercial): NoobAI, FLUX Kontext, MusicGen, Higgs/Fish weights,
    HunyuanVideo (geo-locked), LTX weights.

## The host character — Gemma-san
**Writing/personality bible (read before scripting her): `docs/CHARACTER.md`** — TL;DR: a *cosmic
...
with **Inochi2D** (BSD; the open Live2D alternative — Live2D itself is proprietary,
don't bundle) as the optional smooth-deformation upgrade.

## Conventions
- **Video projects live in `../slopstudio-projects`** (their OWN local-only git repo —
  monorepo, one dir per video, portable project dirs with project-relative `assets/…`
  uris). **Commit project changes THERE, in the same session.** This code repo's master
  is **code-only** (orphan history since 2026-07-02; the full pre-split history is the
  local branch `master-legacy-full`). Run tools from THIS repo root with relative paths:
